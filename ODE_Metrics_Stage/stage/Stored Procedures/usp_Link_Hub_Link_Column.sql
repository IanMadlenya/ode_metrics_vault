...
BEGIN
	...
	;WITH sHubKey	AS (SELECT * FROM [ODE_Metrics_Vault].[RawSat].[s_DV_Hub_Key]			WHERE [dv_row_is_current] = 1	AND [dv_is_tombstone] = 0)
	...
	,sSatellite		AS (SELECT * FROM [ODE_Metrics_Vault].[RawSat].[s_DV_Satellite]			WHERE [dv_row_is_current] = 1	AND [dv_is_tombstone] = 0)
